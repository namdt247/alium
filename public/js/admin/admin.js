...
function ConfirmSupplierActive(event) {
    event.preventDefault();
    if (confirm("Bạn có chắc chắn muốn kích hoạt xưởng?") )
        window.location = event.target.href;
}
...
$(document).ready(function(){
    ...
    $("#imgFeature2").change(function(){
        var file = this.files[0];
        var reader = new FileReader();
        reader.onload = function (e) {
            $("#_imgFeature2").attr("src", e.target.result).css("display","block");
        }
        reader.readAsDataURL(file);
    });

    $("#imgFeature3").change(function(){
        var file = this.files[0];
        var reader = new FileReader();
        reader.onload = function (e) {
            $("#_imgFeature3").attr("src", e.target.result).css("display","block");
        }
        reader.readAsDataURL(file);
    });
