...
class AdminController extends Controller
{
    ...
    public function postEditConfig(Request $request){
        ...
        if($errorCode == _ApiCode::SUCCESS)
            return redirect()->back()->with(['success_message' => 'Chỉnh sửa cấu hình thành công']);
        else
            return redirect()->back()->with(['error_message' => 'Chỉnh sửa cấu hình không thành công']);
    }
    ...
    public function postEditConfigLocale(Request $request){
        $errorCode = $this->_bzAdmin->postEditConfigLocale($request);
        if($errorCode == _ApiCode::SUCCESS)
            return redirect()->back()->with(['success_message' => 'Chỉnh sửa trang tĩnh thành công']);
        else
            return redirect()->back()->with(['error_message' => 'Chỉnh sửa trang tĩnh không thành công']);
    ...
    public function postAddConfigItem(Request $request){
        return $this->_bzAdmin->postAddConfigItem($request);
    }
