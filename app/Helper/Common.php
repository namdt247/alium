...
//        return time().uniqid(true);
    ...
    public static function calPriceAssignee($quality,$quantity,$priceUnit){
        ...
        if ($quality == $lastQuality['id'])
            $assigneePrice = $priceUnit * (1/1.25);
        elseif ($quantity < 100) $assigneePrice = $priceUnit * (1/1.3);
        else $assigneePrice = $priceUnit * (1/1.25);
        if ($priceUnit < 1000) {
            return round($assigneePrice, -2);
        }
        return round($assigneePrice, -3);
    }
}